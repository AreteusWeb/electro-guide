 ...
 * blobs (green / orange / magenta / blue) in the torso ROI.
 */
export function detectStickerCircles(
  ...
  const scale = WORK_WIDTH / roi.width;
  const w = WORK_WIDTH;
  ...
  const blobs: { x: number; y: number; count: number; minX: number; maxX: number; minY: number; maxY: number }[] = [];
